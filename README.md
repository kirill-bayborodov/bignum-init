...
[![C/ASM CI](https://github.com/kirill-bayborodov/bignum-init/actions/workflows/ci.yml/badge.svg)](https://github.com/kirill-bayborodov/bignum-init/actions/workflows/ci.yml)
[![GitHub release (latest by date)](https://img.shields.io/github/v/release/kirill-bayborodov/bignum-init?label=release)](https://github.com/kirill-bayborodov/bignum-init/releases/latest)



`bignum-init` is a high-performance, standalone module for performing a initialisation of value on an arbitrary-precision integer (`bignum_t`).
A highly optimized x86-64 assembly implementation of a bignum initialisation operation, designed for performance-critical applications. 
...
-   **Component:** This project requires `bignum-core` as a git submodule located at `libs/bignum-core`.

To clone the repository with its submodule, use:
```bash
git clone --recurse-submodules https://github.com/kirill-bayborodov/bignum-init.git
```
## API

The library provides a single function, declared in `include/bignum_init.h`.

```c
bignum_init_status_t bignum_init(bignum_t* x);
```
-   **`x`**: A pointer to the `bignum_t` structure to be shifted.
-   **Returns**: A `bignum_init_status_t` enum (`BIGNUM_INIT_SUCCESS`, `BIGNUM_INIT_ERROR_NULL_ARG`).
...
This project produces an object file (`bignum_init.o`) which you can link with your own application.
...
make build
```
The output will be located at `build/bignum_init.o`.
...
```bash
gcc your_app.c build/bignum_init.o -I./include -I./libs/bignum-core/include -o your_app -no-pie
```	
